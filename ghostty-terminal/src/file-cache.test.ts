...
describe('file-cache', () => {
  ...
  describe('quoteShellPath', () => {
    ...
    describe('Windows (isWindows=true)', () => {
      it('leaves simple paths unquoted', () => {
        expect(quoteShellPath('C:\\Users\\file.ts', true)).toBe('C:\\Users\\file.ts');
        expect(quoteShellPath('D:/Projects/app.js', true)).toBe('D:/Projects/app.js');
      });

      it('quotes paths with spaces using double quotes', () => {
        expect(quoteShellPath('C:\\Program Files\\app.exe', true)).toBe('"C:\\Program Files\\app.exe"');
      });

      it('quotes paths with special cmd.exe characters', () => {
        expect(quoteShellPath('C:\\path&name\\file.ts', true)).toBe('"C:\\path&name\\file.ts"');
        expect(quoteShellPath('C:\\path|pipe\\file.ts', true)).toBe('"C:\\path|pipe\\file.ts"');
        expect(quoteShellPath('C:\\(parens)\\file.ts', true)).toBe('"C:\\(parens)\\file.ts"');
      });

      it('escapes double quotes within quoted paths', () => {
        expect(quoteShellPath('C:\\path"with"quotes\\file.ts', true)).toBe('"C:\\path^"with^"quotes\\file.ts"');
      });

      it('does not quote backslashes (literal in cmd.exe)', () => {
        // Backslashes are path separators in Windows, not escape chars
        expect(quoteShellPath('C:\\simple\\path.ts', true)).toBe('C:\\simple\\path.ts');
      });
    });
  });
